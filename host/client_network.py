...
class Host:
	...
	def get_base_interface(self):
		if self._base_interface is None:
			interfaces = [x for x in netifaces.interfaces() if "lo" not in x and "host" not in x ]
	...
	def get_ips_from_control(self):
		...
		"""{
			dev_iface = [{iface: 'eth0', addr: '', netmask: '', broadcast: '' }]
			dev_tunnel = [
					{ iface: 'host0', ip_local: '' ip_remote: '' key:'' }
						 ]
			route = [
	...
	def set_routes(self, route_list):
		...
		for route in route_list:
			if  'via' in route:
				...
				net, mask =  net_tmp[0], net_tmp[1] if len(net_tmp)>1 else "32"
